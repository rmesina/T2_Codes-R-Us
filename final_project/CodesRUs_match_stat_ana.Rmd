...
Now we want to see whether we can incorporate these variables without worrying about multicollinearity. We may be able to do that using PCA/PCR.

```{r PCA}
teams_cont_x <- subset(teams_cont, select = -c(goals))
teams_cont_scaled <- data.frame(scale(teams_cont_x))
...
fit_pc <- lm(goals~., data = teamsscaled.pc)
ezids::xkabledply(fit_pc, title = "Limited model using Principal Comps")
summary(fit_pc)
