...
STEP 2 is to center the values of the independent variables by subtracting the mean of each variable from the each observation of that variable.
```{r step 2}
...
In STEP 3 we built different logit models. It is important to note that we did not use regular feature selection due to the interactions that some of these models would later have. Rather, we implemented a more artisanal trial-and-error method to come up with a model with a low VIF, and a low BIC. This was in fact an iterative process, complementing this step with STEP 4.
...
For STEP 4, we compared the BIC value of each model and checked for multicollinearity by comparing VIF values. As we said before, the target was to find a model with acceptable VIF values, and a low BIC.
...
Model6 has a high BIC, but it doesn't show multicollinearity (vif < 2). Thus, this is the model we will use to extract the probability of success vector and merge it into our original dataset.
.
```{r step 5}
...
Since we are interested in using all match summary variables and there are only ten independent ones in our data frame, we decided to skip feature selection. Since our dependent variable is categorical (team 1 or team 2) we used a logistical regression model. Looking at the p-value of each variable, shown in Tabe x, only 5 out of the 11 variables were found to have any significance. These were: shot.dif, shot.target.dif, pass.acc.dif, corners.dif, and red.dif. This means that the match summary holds a lot of information that is irrelevent to predicting the outcome of the game. Therefore, we made a second model using only the match summary variables that we found to be significant. The summary of model 2 is also shown in Table x. Analyzing these models by comparing their AIC values we see there is no difference between them, backing up our claim that only half of the variables given in a match summary help portray the result of the game.
...
While we can say only half the data given in the match summary helps predict the outcome of a game, we don't know how well that data is able to help us predict the match winner. It also does not mean that the match summary holds all the information necessary to accurately predict a match winner. To do this we need to first train, then test our model. To do this we created training and testing data sets from our match summary data frame. 

To train model 2, we created train and test data sets in 80-20 split of the match-diff data frame. The resulting confusion matrix is shown in Table 4. 
```{r train and test}
...
recall=(214+562)/776
```
![](./JPEGS/winner_conf_matrix.jpg)
**Table 4.** Confusion matrix of test results for winner_model2.

We finished assessing our model by looking  by looking at the ROC plot of our test results and found the area under the curve (AUC) was found to be 0.804, indicating that our model does a good job predicting a winner, but is not overfitted to the training data set.
```{r ROC plot and AUC}
...
plot(model_goals)
```
**Figure 6.** Plots showing the residuals of our model on the training data.

The first thing we notice is the pattern of diagonals in the top left plot. This is an effect of the fact that the dependent variable (# of goals) is a count variable rather than a continuous variable. Later in the report, we explore logistical regression and  Poisson modeling, which are more appropriate for count data (Section 6.5).

Returning to our best linear regression model, we tested our model using the test data set and looked at both the RMSE, and R^2 values, which are 5.259, and 0.416, respectively. The figure below shows predicted number of goals vs the expected number of goals. The red line shows the fitted line. Ideally, this would have a slope of 1, but that is not the cause showing that our model has room for improvement.
...
```
**Figure 7.** Predicted number of goals vs expected number of goals.

### 6.3 Developing a Logistical Regression Model to Predict Expected Goals

From looking at the results of our linear regressionn model we saw that this really was not an appropriate model for predicting expected goals, as we get results such as a negative and non-integer predicted values for expected goals which doesn't represent reality. Therefore, we treated expected goals as a categorical variable and developed a logistical regression model with the same variables as our linear model.
...
Figure 7 shows the confusion matrix resulting from the test results of our multinomial logistical regression model. From the confusion matrix we calculate our accuracy to by 43% which is quite low. From this we can conclude that while the match summary statisticas are sufficient to  predict the winner of the match, their ability to predict the specific score is quite poor and if we want to be able to accurately predict the number of goals we need to identify different variables.
...
```
**Figure 8** Confustion matrix showing the results of our multinomial logistical regression model.
...
```
**Figure 9.** Shows RMSEP vs number of PC components.
...
```{r limited pc model poisson}
fit_pc_lim_poissonmodel <- glm(goals~PC1+PC2+PC7+PC9, family = "poisson", data = teamsscaled.pc)

#summary(fit_pc_lim_poissonmodel)

ezids::xkabledply(fit_pc_lim_poissonmodel, title = "Poisson model using Principal Comps")

```

Applying Poisson modelling to the best linear regression model (identified under Adjusted R Squared and BIC criteria) and the limited PCR model, we observe the following:

1. Between the linear regression and poisson modelling, the relative magnitude and signs of the coefficients did not change. All variables remain significant.
2. The linear regression has a rather low Adjusted R squared, and the Poisson model has a rather high AIC. Both don't perform as well as we'd like, but the two models show us consistent results.
3. Between the linear PCR and Poisson PCR, we see similar differences.

We conclude that because 1) PCR is able to account for the most features in the data without concern for multicollinearity, and 2) Poisson is more appropriate for count data than a linear regression, the best model to estimate number of goals is the PCR-Poisson model.

However, we know that all four models do not exactly perform as well as we would like. There is a lot of variation in number of goals that's being left unexplained. Perhaps lack of data is the bigger challenge. This makes sense because there are quite a few factors that would intuitively impact the number of goals (or the chance of winning) such as being the home/away team.

## Chapter 7: Conclusions

1. 

2. Only half of the match summary variables are necessary to accurately predict the winner of a match quite accurately, with our model having an accuracy of 73%.

3. The best model to estimate number of goals, given available data, is the Poisson-PCR modelling. All other "best" models perform similarly but Poisson is more appropriate for count data, and PCR accounts for many features without having to worry about multicollinearity. Better and more complete data will likely further improve the model performance.