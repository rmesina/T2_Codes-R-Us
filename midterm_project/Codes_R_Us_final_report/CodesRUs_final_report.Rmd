...
```{r modelling shot success, include=FALSE}
...
merge_model$Tag.6 <- ifelse(merge_model$Tag.5 == "Accurate" | is.na(merge_model$Tag.5) | merge_model$Tag.5 == "Not Accurate",
                           "Other", "Accurate")

merge_model$Success <- rep(0, times=nrow(merge_model))
...
model1 <-lm(Success ~ x.Start + y.Start, data = merge_model)
model2 <- lm(Success ~ x.Start + x.End + y.Start + y.End, data = merge_model)
model3 <- lm(Success ~ x.Start + x.End + y.Start + y.End + Distance, data = merge_model)
...
```
To model successful goals, we had to assign a Boolean to each event, determining if the event resulted in a successful conversion to a goal or not: we assigned a value of 1 (one) to those events which were successful, and a value of 0 (zero) otherwise.
We also made an estimation of the distance that the ball travelled with each event by using the typical field FIFA dimension of 120 by 75 yards. Applying this to our X,Y coordinates, we then used the Pythagoras theorem. In essence:
Distance= √((1.2(x_end-x_start ))^2+(0.75(y_end-y_start ))^2 )

Finally, we proposed three models to determine a probability of success for each variable of interest.

Model 1
The probability of success is a function of the starting coordinates of X, Y.
P[S]=β_0+β_1 x_start+ β_2 y_start  

Model 2
The probability of success is a function of the starting and ending coordinates of X, Y.
P[S]=β_0+β_1 x_start+ β_2 y_start+β_3 x_end+ β_4 y_end  

Model 3
The probability of success is a function of the starting and ending coordinates of X, Y, and the distance traveled by the ball for each event.
P[S]=β_0+β_1 x_start+ β_2 y_start+β_3 x_end+ β_4 y_end+β_5 Distance  

The following table shows the summary of each model. As the Adjusted R2 and BIC coefficients show, Model 3 is the best fitted model.

![](./JPEGS/model_table.jpeg)

Please note that these estimates are not the actual probability of success (a goal being scored). Rather, they represent useful information that we will use in future analysis to portray the actual probabilities of success.
Some things that we will consider for next steps are that the training of the model is unbalanced (i.e., there many more failures than successes), and we will need a logit regression function to adapt this models.
...
Lastly, from the progress we've made developing our logistical regression model, we can conclude that all variables we have selected are significant in improving the model. Further development of this model is to be pursued.
