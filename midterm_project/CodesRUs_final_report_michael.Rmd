...
The distance distributions for key passes and assists are displayed in Figure 3. Both show fairly normal distributions, despite the fact that neither the x,y-coordinates for these two events show normal distributions.
...
ak_hist_d<-ggplot()+
  ...
  labs(title="Distance of Assists and Key Passes", x="Distance", y="Normalized Count")+
...
The t-tests were done assuming a normal distribution of the event variables despite most of the distributions not being normal. This was quick and easy, even though there's room for error it still allows us to get a sense of the differences in variables the determine success or failure.
...
assist_wc<-subset(worldcup, Tag.1=='Key pass')
keypass_wc<-subset(worldcup, Tag.1=='Assist')
assist_keypass_wc<-rbind(assist_wc, keypass_wc)

#convert x and y coordinate columns from characters to integer values
assist_keypass_wc$x.End <- as.numeric(assist_keypass_wc$x.End)
assist_keypass_wc$y.End <- as.numeric(assist_keypass_wc$y.End)

assist_keypass_wc$x.Start <- as.numeric(assist_keypass_wc$x.Start)
assist_keypass_wc$y.Start <- as.numeric(assist_keypass_wc$y.Start)

#convert event, subevent, tags, players, team labels, etc... to factors
assist_keypass_wc$Event.Name=as.factor(assist_keypass_wc$Event.Name)
assist_keypass_wc$Subevent.Name=as.factor(assist_keypass_wc$Subevent.Name)
assist_keypass_wc$Match.Period=as.factor(assist_keypass_wc$Match.Period)
assist_keypass_wc$Match.Date=as.factor(assist_keypass_wc$Match.Date)
assist_keypass_wc$Player=as.factor(assist_keypass_wc$Player)
assist_keypass_wc$Team.Name=as.factor(assist_keypass_wc$Team.Name)
assist_keypass_wc$Tag.1=as.factor(assist_keypass_wc$Tag.1)
assist_keypass_wc$Tag.2=as.factor(assist_keypass_wc$Tag.2)
assist_keypass_wc$Tag.3=as.factor(assist_keypass_wc$Tag.3)
assist_keypass_wc$Tag.4=as.factor(assist_keypass_wc$Tag.4)
assist_keypass_wc$Tag.5=as.factor(assist_keypass_wc$Tag.5)
assist_keypass_wc$Tag.6=as.factor(assist_keypass_wc$Tag.6)

assist_keypass_wc<-assist_keypass_wc %>% mutate(dist = sqrt((x.End-x.Start)^2+(y.End-y.Start)^2))
assist_keypass_wc$dist <- as.numeric(assist_keypass_wc$dist)

freq1 <- sort(table(assist_keypass_wc$Team.Name),decreasing=T)

# Stack shot and goal dataset
shots_wc<-subset(worldcup, Event.Name=='Shot')
goals_wc<-subset(worldcup, Tag.1=='Goal')
shots_goals <-rbind(shots_wc, goals_wc)
shots_goals_wc<- subset(shots_goals, competition=="World Cup")

#convert x and y coordinate columns from characters to integer values
shots_goals_wc$x.End <- as.numeric(shots_goals_wc$x.End)
shots_goals_wc$y.End <- as.numeric(shots_goals_wc$y.End)

shots_goals_wc$x.Start <- as.numeric(shots_goals_wc$x.Start)
shots_goals_wc$y.Start <- as.numeric(shots_goals_wc$y.Start)

#convert event, subevent, tags, players, team labels, etc... to factors
shots_goals_wc$Event.Name=as.factor(shots_goals_wc$Event.Name)
shots_goals_wc$Subevent.Name=as.factor(shots_goals_wc$Subevent.Name)
shots_goals_wc$Match.Period=as.factor(shots_goals_wc$Match.Period)
shots_goals_wc$Match.Date=as.factor(shots_goals_wc$Match.Date)
shots_goals_wc$Player=as.factor(shots_goals_wc$Player)
shots_goals_wc$Team.Name=as.factor(shots_goals_wc$Team.Name)
shots_goals_wc$Tag.1=as.factor(shots_goals_wc$Tag.1)
shots_goals_wc$Tag.2=as.factor(shots_goals_wc$Tag.2)
shots_goals_wc$Tag.3=as.factor(shots_goals_wc$Tag.3)
shots_goals_wc$Tag.4=as.factor(shots_goals_wc$Tag.4)
shots_goals_wc$Tag.5=as.factor(shots_goals_wc$Tag.5)
shots_goals_wc$Tag.6=as.factor(shots_goals_wc$Tag.6)

shots_goals_wc<-shots_goals_wc %>% mutate(dist = sqrt((x.End-x.Start)^2+(y.End-y.Start)^2))
shots_goals_wc$dist <- as.numeric(shots_goals_wc$dist)
...
```{r boxplots of wc top five, fig.show='hold', out.width='33%'}
assist_keypass_wc$Team.Name <- as.character(assist_keypass_wc$Team.Name)

assist_keypass_wc_top5 <- subset(assist_keypass_wc, Team.Name == "Belgium" | Team.Name == "England" | Team.Name == "Croatia" | Team.Name == "Brazil" | Team.Name == "France")

ak_box_x_wc<-boxplot(assist_keypass_wc_top5$x.Start ~ assist_keypass_wc_top5$Team.Name,
...
ak_box_y_wc<-boxplot(assist_keypass_wc_top5$y.Start ~ assist_keypass_wc_top5$Team.Name, 
...
ak_box_dist_wc<-boxplot(assist_keypass_wc_top5$dist ~ assist_keypass_wc_top5$Team.Name, 
...
![source: https://www.presentationgo.com/presentation/knockout-stage-powerpoint-google-slides/](./JPEGS/2018-Fifa-World-Cup-Knockout-Stage.png)  
