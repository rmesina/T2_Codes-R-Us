...
The t-tests were done assuming a normal distribution of the event variables despite most of the distributions not being normal. This was quick and easy, even though there's room for error it still allows us to get a sense of the differences in variables that determine success or failure.
...
1. Using t-tests to compare configurations between assists and key passes, and separately, goals and shots, the team found statistically significant differences in means of the following:

  a. Starting x-coordinate of assists vs key passes. In particular, this           result shows that, on average, final passes made further up the field are      more likely to result in an assist than a key pass.

  b. Distance covered by assists vs key passes. In particular, this shows that      final passes that have to cover a shorter distance are more likely to         result in an assist.
  
  c. Starting x-coordinate of shots vs goals. In particular, we can conclude      that attempts on goal taken further up the field are more likely to result     in a goal.
  
2. From the probability plots on the same variables of interest, the team concludes the following:

  a. First, goal probability varies with respect to all but one of the            variables, which is the start x-coordinate of the final pass.  
  b. Second, goal probability shows the greatest change as functions of final       pass distance and shot x-coordinate. That is, the probability of a goal       appears most sensitive to these two variables. 

3. Using ANOVA to test for differences in strategies among 5 teams in the World Cup (Belgium, England, Croatia, Brazil, and France), the team found some statistically significant differences among these teams with respect to the following:

  a. Starting x-coordinate of final passes
  b. Starting x-coordinate of shots
  c. ANOVA does not give information as to which pairs of teams are different     from each other, but we can conclude that at least some of the 5 teams,       when compared to others, have different strategies when it comes to           their starting points for passes and shots.
  
4. [PLACEHOLDER FOR MODELLING CONCLUSIONS]

## Bibliography
Pappalardo, Luca; Cintia, Paolo; Rossi, Alessio; Massucco, Emanuele; Ferragina, Paolo; Pedreschi, Dino; et al. (2019): Metadata record for: A public data set of spatio-temporal match events in soccer competitions. figshare. Dataset. https://doi.org/10.6084/m9.figshare.9711164.v2 
  
Nielson Sports (2018): World Football Report, 2018. www.nielsen.com/wp-content/uploads/sites/3/2019/04/world-football-report-2018.pdf
